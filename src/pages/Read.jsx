...
import { getDataOnCookie, setDataOnCookie, bookmark, deleteBookmark, hasBookmarked, generateKey } from '../core/functions'
...
const Read = () => {
  ...
  let [hasBookmark, setHasBookmark] = useState(surah ? hasBookmarked(surah.number) : hasBookmarked("00"));
  
  useEffect(() => {
    axios.get(`https://quran-api-32ouuhnph-renomureza.vercel.app/surahs/${surahid}`)
    .then(result => {
      let surah = result.data;
      surah.number = `${surah.number.toString().length < 2 ? "0" + surah.number : surah.number}`
